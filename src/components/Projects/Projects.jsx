...
class Projects extends Component {
  ...
  toggleCategories(){   
    const ReactItems = CardsData[0].map(item => <MyCard key ={item.id} {...item} />);
    const CsItems =  CardsData[1].map(item => <MyCard key ={item.id} {...item} />);
    const NodeItems = CardsData[2].map(item => <MyCard key ={item.id} {...item} />);
  ...
  render(){
    return(
      <div className="category-tabs">
        ...
        <div className="button__add--container">
        </div>
        <Grid className="wrap" >         
          <Cell col={12} >   
